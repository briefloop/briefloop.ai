...
for en in (False,True):
 ...
 def shell(title,body,slug='index'):
  ...
  desc=f"{e(title)} — {'Practical BriefLoop guides for installation, reports, sources and data.' if en else 'BriefLoop 可操作的安装、写作、依据和数据使用指南。'}"
  url=f'https://briefloop.ai/docs/{link(slug)}'
  social=f'<meta name="theme-color" content="#faf9f6"><meta property="og:type" content="article"><meta property="og:site_name" content="BriefLoop"><meta property="og:locale" content="{"en_US" if en else "zh_CN"}"><meta property="og:url" content="{url}"><meta property="og:title" content="{e(title)} · BriefLoop"><meta property="og:description" content="{desc}"><meta property="og:image" content="https://briefloop.ai/assets/workbench-home-20260914.png"><meta property="og:image:width" content="2640"><meta property="og:image:height" content="1800"><meta name="twitter:card" content="summary_large_image">'
  return f'''<!doctype html><html lang="{lang}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{e(title)} · BriefLoop</title><meta name="description" content="{desc}"><link rel="icon" href="../assets/briefloop-icon.svg"><link rel="stylesheet" href="docs.css">{site_header.head('../')}<link rel="canonical" href="{url}">{social}<link rel="alternate" hreflang="zh-CN" href="https://briefloop.ai/docs/{slug}.html"><link rel="alternate" hreflang="en" href="https://briefloop.ai/docs/{slug}.en.html"></head><body class="{"guides-index" if slug=="index" else "guide-article"}"><a class="skip" href="#content">{'Skip to content' if en else '跳到正文'}</a>{site_header.header(en,alternate,'../','guides')}<div class="layout"><aside><p class="eyebrow">{'USER GUIDES' if en else '使用文档'}</p><nav aria-label="{'Guide topics' if en else '文档目录'}">{nav}</nav></aside><main id="content">{body}</main></div><footer>BriefLoop · <a href="../downloads{suffix}.html">{'Published installers and requirements' if en else '已发布安装包与要求'}</a> · <a href="https://github.com/Stahl-G/briefloop/releases">{'Release notes' if en else '发行说明'}</a><p>{'Workspaces stay local. Cloud models and search services receive relevant content when used.' if en else '工作区保存在本地；调用云端模型或搜索时，相关内容会发送给你选择的服务。'}</p></footer>{site_header.script('../')}<script src="search.js" defer></script></body></html>'''
 search=[]
 for x in items:
